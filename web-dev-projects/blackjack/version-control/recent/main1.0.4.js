    ...
    /*
    August 17, 2022
    I'm trying to store the API call to the deck of cards locally; so I dont have to make
    multiple API calls
    */
    var playerHand = [];
    var dealerHand = [];
    var discardPile = [];
    var deck; // after the API call, store the data locally into the program
    ...
    startGameBtn.addEventListener('click', () => {
        ...
        request.onreadystatechange = function () {
            ...
            if (request.readyState === XMLHttpRequest.DONE && request.status === 200) {
                ...
                //console.log( "your deck contains:" + data);
                //console.log(data);

                deck = data;
                console.log("our deck containers:" + deck);
                console.log(deck);
                deckId = data.deck_id;    
                ...
                // dealerHand[0] has the first value
                if (dealerHand[0].value == 'ACE') {
                    dealerHandVal = 11;
                    dealerHandVal.innerText = parseInt(dealerHandVal);
                    dealerAceFlag = true;
                }
                else if ( dealerHand[0].value == 10 || dealerHand[0].value == 'KING' || dealerHand[0].value == 'QUEEN' || dealerHand[0].value == 'JACK' ) {
                    // parse KING, QUEEN, JACK, 10
                    dealerHasTen = true;
                    dealerValue = 10;
                    dealerHandVal.innerText = 10;
                } else {
                    // value is 2,3,4,5,6,7,8,9
                ...
                // dealerHand[1] has the 2nd value
    ...
    // end of startgame()
        ...
        */
        ...
        function checkHand (takeHandArray) {
            ...
            takeHandArray.forEach( (card) => {
                ...
                if ( handValue > 21) {
                    ...
                    gameoverscreen('You lose! You busted!');
                }
            })
        ...
        function handleAi( myArray ) {
            ...
            dealerHandVal.innerText = "";
            dealerHandVal.innerText = count;
            dealerValue = 0;
            dealerValue = count;
            // end the game
            if ( dealerValue >= 17 ) {
                // dealer stays at values >= 17
                compareResults();
                return;
            }
        ...
        function gameoverscreen(message) {
            var elem = document.querySelector('.gameover');
            elem.style.display = "block";
            var outputMessage = document.getElementById('whoWinsCaption');
                ...
                if (request.readyState === XMLHttpRequest.DONE && request.status === 200) {
                    ...
                    if ( dealerAceFlag ) {
                        if (dealerHand[1].value == 'KING' || dealerHand[1].value == 'QUEEN' || dealerHand[1].value == 'JACK' || dealerHand[1].value == 10 ) {
                            ...
                            dealerHandVal.innerText = parseInt(dealerValue);
                            winnerCaption.innerText = 'Computer wins! BLACKJACK!';
                            gameoverscreen('Dealer pulled Blackjack!');
                            return;
                        }
                    }
